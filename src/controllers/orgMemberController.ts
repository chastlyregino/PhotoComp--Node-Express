...
orgMemberRouter.delete(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            if (userId === adminId) {
                throw new AppError('Administrators cannot remove themselves from the organization', 400);
            }
...
orgMemberRouter.patch(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const updatedMember = await orgService.updateMemberRole(orgName, userId, role as UserRole);
