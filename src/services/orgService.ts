...
import { Organization, OrganizationCreateRequest, createOrganization, OrganizationUpdateRequest, updateOrganization, UserOrganizationRelationship, createOrganizationAdmin } from '../models/Organizations';
...
export class OrgService {
    ...
    async createOrg(createOrg: OrganizationCreateRequest, userId: string): Promise<OrganizationCreateRequest | null> {
        try {
            ...
            try {
                ...
                throw new AppError(`Organization creation failed: DB! ${(error as Error).message}`, 500);
            }
        } catch (error) {
            if (error instanceof AppError) {
                throw error;
            }
            throw new AppError(`Organization creation failed: Model! ${(error as Error).message}`, 500);
        }
    }
    ...
    async findOrgById(id: string): Promise<Organization | null> {
        try {
            ...
            throw new AppError(`Finding Organization by ID failed! ${(error as Error).message}`, 500);
        }
    }
    ...
    async findOrgsByUser(userId: string): Promise<Organization[] | null>{
        try {
            ...
            throw new AppError(`Finding Organization by User failed! ${(error as Error).message}`, 500);
        }
    }
